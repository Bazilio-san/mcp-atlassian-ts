...
4) Не запускай эмулятор. Все настроено на девелоперскую jira. Ни вкоем случае не трогай переменные в .env.
...
**Решение:**
Изменить метод `getOrSet` в `src/core/cache.ts`, чтобы он пробрасывал ошибки от factory функции как есть, а не оборачивал их в `CacheError`. Разделить обработку ошибок:
